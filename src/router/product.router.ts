...
export default class ProductRouter {
    ...
    searchProducts = async (req: Request, res: Response) => {
        try {
            let searchParameter = req.query.q;
            let arrayParameters = searchParameter.split(" ");
            let query: any = `${arrayParameters[0]}`;
            for (var i = 1; i < arrayParameters.length; i++) {
                query += `.*${arrayParameters[i]}`;
            }
            query = new RegExp(query);
            const currentDoc = await this.productModel.find({ $or: [{ name: {$regex: query} }, { description: { $regex: query, $options: 'i' } }] });
    ...
    modifyProduct = async (req: Request, res: Response) => {
        try {
            const currentDoc = await this.productModel.findOneAndUpdate({ _id: new ObjectId(req.body._id) }, req.body, { new: true });
            return res.status(200).json(currentDoc);
        } catch (error) {
            return res.status(500).json(error);
        }
